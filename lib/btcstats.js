...
//TODO: quick hack - remove manually until i update xchange.js
delete xchange["Xchange"];
delete xchange["addApis"];
delete xchange["btc38"];	//remove until btc38 api is fixed
	...
	tickersResp.call(this, function(err, resp){
		if(err){
			...
			var totalVol = resp.reduce(function(a, b){
				return parseFloat(a) + parseFloat((b.volume) ? b.volume : 0);
			}, 0);

			var res = resp.reduce(function(a, b){
				return {
					price: a.price + midpoint(b) * parseFloat((b.volume) ? b.volume : 0) / totalVol 	    		
				}
			}, {price: 0});
...
BtcStats.prototype.minVolume = function(callback) {
	tickersResp.call(this, function(err, resp){
		if(err){
			...
			callback(null, {volume: parseFloat(min.volume).toFixed(2), exchange: min.ticker});
		}
	});
}
...
BtcStats.prototype.maxVolume = function(callback) {
	tickersResp.call(this, function(err, resp){
		if(err){
			...
			callback(null, {volume: parseFloat(max.volume).toFixed(2), exchange: max.ticker});
		}
	});
}
...
BtcStats.prototype.minSpread = function(callback) {
	tickersResp.call(this, function(err, resp){
		if(err){
			...
			var spread = {spread: (parseFloat(min.ask) - parseFloat(max.bid)).toFixed(2), 
					bid: max.bid, 
					ask: min.ask, 
...
BtcStats.prototype.exchangeMaxSpread = function(callback) {
	tickersResp.call(this, function(err, resp){
		if(err){
			...
			var spread = {spread: (parseFloat(max.ask) - parseFloat(max.bid)).toFixed(2), 
					bid: max.bid, 
					ask: max.ask, 
					exchange: max.ticker};
...
BtcStats.prototype.exchangeMinSpread = function(callback) {
	tickersResp.call(this, function(err, resp){
		if(err){
			...
			var spread = {spread: (parseFloat(min.ask) - parseFloat(min.bid)).toFixed(2), 
					bid: min.bid, 
					ask: min.ask, 
					exchange: min.ticker};
